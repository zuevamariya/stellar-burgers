import { fetchIngredients } from '../ingredients/ingredients-action';
...
describe('Проверка редьюсера слайса feeds', () => {
  test('загрузка', () => {
    ...
    expect(pendingState.isIngredientsLoading).toEqual(
      expectedResult.isIngredientsLoading
    );
  });
  ...
  test('успешно', () => {
    ...
    expect(fulfilledState.isIngredientsLoading).toEqual(
      expectedResult.isIngredientsLoading
    );
    expect(fulfilledState.ingredients).toEqual(expectedResult.ingredients);
  });
});
